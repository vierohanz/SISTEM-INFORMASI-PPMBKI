...
export default function Article() {
        ...
        const cleaned = response.data.map(item => {
          ...
          try {
            ...
            image = Array.isArray(parsed)
              ? `https://ppmbki.ponpes.id/storage/${parsed[0].replace(/\\/g, "")}`
              : "";
          } catch {}
          return {
            id: item.id,
            ...
            monthName: months[new Date(item.tanggal_upload).getMonth() + 1],
          };
        });
        setArticles(cleaned);
  ...
  useEffect(() => {
    ...
    if (selectedDivisions.length > 0) {
      filtered = filtered.filter(a => selectedDivisions.includes(a.divisi));
    }
    if (selectedMonths.length > 0) {
      filtered = filtered.filter(a => selectedMonths.includes(a.monthName));
    }
  ...
  return (
    <section>
      ...
      {/* Filter Section */}
      <div className="bg-white py-8 px-6 sm:px-12 border-b border-gray-200">
        <div className="flex items-center gap-6 flex-wrap">
          <div className="flex items-center gap-2">
            <Filter size={20} className="text-gray-600" />
            <span className="text-gray-800 font-semibold text-lg">Filter</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={divMenuRef}>
            <button
              ...
              className="flex items-center gap-2 bg-white rounded-xl px-4 py-2 shadow-sm focus:outline-none"
            >
            ...
            {openDivMenu && (
              <div className="absolute mt-2 w-48 bg-white rounded-xl shadow-lg p-4 z-10">
                {DIVISION_ORDER.map(div => (
                  ...
                  >
                    ...
                    />
                    <span className="text-sm text-gray-800">{div}</span>
                  </label>
                ))}
              </div>
            )}
          ...
          <div className="relative" ref={monMenuRef}>
            ...
            {openMonMenu && (
              <div className="absolute mt-2 w-48 bg-white rounded-xl shadow-lg p-4 z-10 max-h-56 overflow-auto">
                {months.slice(1).map(mon => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={selectedMonths.includes(mon)}
          ...
          <>
            {highlighted && (
              <motion.div
                ...
                onClick={() => navigate(`/article/${highlighted.id}`)}
                ...
                <div className="flex flex-col lg:flex-row">
                  ...
                  <div className="lg:w-1/2 p-8 flex flex-col justify-center">
                    ...
                    <p className="text-gray-600 leading-relaxed">
                      {highlighted.description.slice(0, 300)}...
                    </p>
                    <div className="mt-4 flex items-center text-sm text-gray-500">
                      <Calendar size={16} className="mr-2" />
                      {highlighted.date.toLocaleDateString("id-ID", {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
                      })}
